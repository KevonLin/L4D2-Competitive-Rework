...
#define PLUGIN_VERSION "1.0h-2023/8/10"
...
public void OnPluginStart()
{
	...
	hTSAnnounce = CreateConVar("multitanks_announce", "1", "Enable/Disable Announcements when tank spawns.", FCVAR_NOTIFY|FCVAR_SPONLY);
	hTSDisplay = CreateConVar("multitanks_display", "1", "Enable/Disable Tank HUD Display", FCVAR_NOTIFY|FCVAR_SPONLY);
...
Action SpawnMoreTank(Handle timer)
{
	...
	int anyclient = GetRandomClient(2);
	if(anyclient == 0)
	{
		PrintToServer("There are no any survivor at this moment.");
		return Plugin_Continue;
	}
	...
	for (int i=1;i<=MaxClients;i++)
	{
		if (IsClientInGame(i) && !IsFakeClient(i) && GetClientTeam(i) == 3) 
		{
			...
			if (L4D_IsPlayerGhost(i) || !IsPlayerAlive(i))
			{
	...
	chosenInfected = (iClientCount == 0) ? 0 : iClients[GetRandomInt(0, iClientCount - 1)];

	if(chosenInfected == 0)
	{
		float vecPos[3];
		if(L4D_GetRandomPZSpawnPosition(anyclient,8,5,vecPos) == true)
		{
	...
	else
	{
		...
		for (int i=1;i<=MaxClients;i++)
		{
			if (i != chosenInfected && IsClientInGame(i) && !IsFakeClient(i) && GetClientTeam(i) == 3)
			{
